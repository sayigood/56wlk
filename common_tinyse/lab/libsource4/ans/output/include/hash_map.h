...
public:
	...
	int create(int hn, hash_fcn_t fcn, compare_t cft = _hash_compare_fun )
	{
		...
		if( ( m_lock = ( pthread_rwlock_t * )malloc( sizeof( pthread_rwlock_t ) ) ) == 0 ){
			_ANS_WARNING_LOG("malloc lock error");
			return -1;
		}
		if( pthread_rwlock_init( m_lock, NULL ) != 0 ){
			_ANS_WARNING_LOG("init rwlock fail");
			free(m_lock);
			m_lock = 0;
			return -1;
		}
...
#ifdef HASH_MONITOR_X
	void monitor(){
	    _ANS_NOTICE_LOG("[monitor]--------------------");
	    _ANS_NOTICE_LOG("[monitor]bucket[%d/%d]",
		    used_bucket,
		    m_bucket_num);
	    _ANS_NOTICE_LOG("[monitor]nodes[%d]",
		    m_node_num);
	    _ANS_NOTICE_LOG("[monitor]--------------------");
	}
