...
class ConfigReloader {
	typedef std::map <bsl::string, config_monitor_t *> config_monmap_t;
	typedef std::map <bsl::string, config_monitor_t *> :: iterator config_monmap_itr_t;
private:
	int _finit; /**< 初始化标志   */
	...
	std::map <bsl::string, config_monitor_t *> _chrcbmap;  /**<  存储监控的字符key和回调    */
	std::map <bsl::string, config_monitor_t *> _strcbmap;  /**<  存储监控的字符串key和回调    */
	std::map <bsl::string, config_monitor_t *> _intcbmap;  /**<  存储监控的整数key和回调    */
	std::map <bsl::string, config_monitor_t *> _uint64cbmap;  /**<  存储监控的浮点key和回调    */
	std::map <bsl::string, config_monitor_t *> _floatcbmap;  /**<  存储监控的浮点key和回调    */
	std::map <bsl::string, config_monitor_t *> _groupcbmap;  /**<  存储监控的group和回调    */
	 ...
	 * @brief 释放存储的数据
	 *
	 * @param [in] map   : std::map
	 * @return  void
	 **/
	void _free_map(std::map <bsl::string, config_monitor_t *> *map);
	/**
	 * @brief 释放存储的数据
	 *
	 * @param [in] map   : std::map
