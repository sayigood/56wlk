...
public:
	...
	int _normal_erase(size_t key, const key_type &k, node_pointer *__bucket, deleteVec *delvec = NULL) {
		node_pointer lst = __bucket[key];
		node_pointer front = 0;
		while (lst) {
			if (_base._equl(_base._getkey(_base._sp_alloc.getp(lst)->val), k)) {
		...
		{
			{
				...
				for (size_t i=0; i<_hashset->_ht._bitems; ++i) {
					...
					while (_hashset->_ht._bucket[i]) {
						...
						{
							const key_type & key = _base._getkey(*(_hashset->_ht._sp_alloc.getp(nd)->val));
							_normal_erase(_base._hashfun(key) % _base._bitems, key, _base._bucket, &delvec);
						}
						//考虑get的时候 检索可能出的问题
						_hashset->_ht._bucket[i] = _hashset->_ht._sp_alloc.getp(_hashset->_ht._bucket[i])->next;
