	...
	class deque
	{
		...
		private:
			...
			static const size_type _BUFF_SIZE = sizeof(_Tp)<_BuffSize?
				size_type(_BuffSize/sizeof(_Tp)):size_type(1);		  /**< 每个块的元素个数       */
			enum { _INITIAL_MAP_SIZE = 8UL };		  /**< 索引表的大小初始为 _INITIAL_MAP_SIZE       */
			/**
			* @brief  指向头块和尾块的结构体
			*  
			*/
		...
		public:
			...
			**/
			 ...
			 *
			 * @return  成功返回0， 失败返回-1, 如果失败，不能调用deque的其他接口
			**/
			int create()
			{
				//默认初始化 _INITIAL_MAP_SIZE的索引大小
		...
		private:	
			...
			**/
			 ...
			 * @note 不支持= ,同时也不支持 std::swap, swap使用 swap方法
			**/
