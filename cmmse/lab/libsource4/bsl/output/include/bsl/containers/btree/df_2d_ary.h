...
{ \
    int64_t j = 0; \
    const int64_t n = (int64_t)data_len; \
    if (NULL != (info)) \
    { \
        DF_WRITE_LOG_US(log_level, "%s,len=%ld", info, n); \
    } \
    if (NULL != (buf) && n > 0) \
    { \
        while (j < n) \
...
public:
    ...
    void set_in_use(void) {
        atomic_exchange(&_ref_counter_next_index, 0);
    };
    ...
    void set_free(void) {
        ...
        atomic_exchange(&_ref_counter_next_index, FREE_FLAG);
    };
    //
    ...
    int add_to_list(const uint64_t next_index, dfs_2d_base_t & next_item) {
        ...
        if (is_in_use()) {
            ...
            } while(atomic_compare_exchange(&_ref_counter_next_index, nv, cv) != cv);
        }
    ...
    int remove_from_list(const uint64_t cv, const uint64_t third_index, bool & is_removed) {
        ...
        if (is_in_use()) {
            ...
            if(atomic_compare_exchange(&_ref_counter_next_index, nv, cv) == cv) {
                is_removed = true;
            }
        }
...
public:
    ...
    int inc_ref_counter(uint64_t & ref_counter) {
        ...
        if ((ref_counter = atomic_inc(&_ref_counter_next_index)) >= MAX_REF_COUNTER) {
            log_level = DF_UL_LOG_FATAL;
    ...
    int inc_ref_counter_if_not_zero(uint64_t & ref_counter) {
        ...
        while (org_v != 0 && (org_v & FREE_FLAG) == 0) {
            pre_v = atomic_compare_exchange(&_ref_counter_next_index, org_v+1, org_v);
            if (pre_v == org_v) {
    ...
    int dec_ref_counter(uint64_t & ref_counter) {
        ...
        if ((ref_counter = atomic_dec(&_ref_counter_next_index)) >= MAX_REF_COUNTER) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_ref_counter >= MAX_REF_COUNTER");
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_REF_COUNTER);
            ref_counter = UNDEF_REF_COUNTER;
        ...
        if ((_t_index_for_next_row/ROW_SIZE) >= _t_row_ary_size) {
            ...
            if (NULL == new_t_row_ary) {
                ...
                for (; j < _t_row_ary_size; ++j) {
                    //atomic_exchange_pointer((volatile pvoid *)&(new_t_row_ary[j]), (pvoid)_t_row_ary[j]);
                    new_t_row_ary[j] = _t_row_ary[j];
                }
                for (; j < new_t_row_ary_size; ++j) {
                    new_t_row_ary[j] = NULL;
                }
                old_t_row_ary = (T **)atomic_exchange_pointer(
                                    (volatile pvoid *)&(_t_row_ary),
                                    (pvoid)new_t_row_ary);
                old_t_row_ary_size = atomic_exchange(
...
private:
    int _get_exist_du(const uint64_t t_index, PDU & pdu) const {
...
public:
    ...
    int update_cur_index(const dfs_rc_indexer_t & new_rc_indexer, dfs_rc_indexer_t & old_rc_indexer) {
        ...
        //}
            ...
            old_cur_index = atomic_exchange(&_cur_index, new_cur_index);
    ...
    //说明：需要保证获得的项没有被回收
    int retrieve_cur_index(dfs_rc_indexer_t & rc_indexer) const {
...
public:
    dfs_bt2d_ary_t() {
        ...
        for (uint32_t j = 0; j < len_of_ary(_realloc_t_row_ary); ++j) {
...
#ifdef  DF_BT_STATISTIC_INFO
        atomic_exchange(&_obj_del_counter, 0);
        atomic_exchange(&_obj_add_counter, 0);
        atomic_exchange(&_obj_ref_inc_counter, 0);
        atomic_exchange(&_obj_ref_dec_counter, 0);
#endif
        return;
    };
    ...
    int _enlarge_row_ary(void) {
        ...
        if ((_t_index_for_next_row/ROW_SIZE) >= _t_row_ary_size) {
            ...
            if (NULL == new_t_row_ary) {
                ...
                old_t_row_ary = (dfs_2d_du_t<T> **)atomic_exchange_pointer(
                                    (volatile pvoid *)&(_t_row_ary),
                                    (pvoid)new_t_row_ary);
                old_t_row_ary_size = atomic_exchange(
                                         (volatile uint64_t *)&_t_row_ary_size,
                ...
                _realloc_counter = (_realloc_counter+1)%len_of_ary(_realloc_t_row_ary);
            }
        }
...
//        else
//        {
//            atomic_exchange(&_mutation_counter, mutation_counter);
//        }
//
//        return err;
//    };
...
//    {
//        atomic_exchange(&_syn_mutation_counter, _mutation_counter);
//        return;
//    };
//    void start_copying(const dfs_mutation_sycnizer_t & src)
//    {
//        atomic_exchange(&_syn_mutation_counter, src._syn_mutation_counter);
//        return;
//    };
//    void end_copying(const dfs_mutation_sycnizer_t & src)
//    {
//        atomic_exchange(&_mutation_counter, src._mutation_counter);
//        return;
//    };
//    //数据是否完整(vs.正在修改中)
