	...
	*/
	...
	class rwseque
	{
		...
		private:
			...
			typedef typename _InnerAlloc::pool_type::template rebind<value_type*>::other _base_pointer;
			typedef typename _InnerAlloc::pool_type::template rebind<pthread_rwlock_t>::other _base_lock; 
			typedef typename _InnerAlloc::pool_type::template rebind<pthread_rwlock_t*>::other _base_lock_list; 
		...
		public:
			...
			int create()
			{
				return create(size_type(_INITIAL_MAP_SIZE));
			}

			int create(size_type __size)
			{
				destroy();
			...
			**/
			...
			int _assign(_Integer __n, _Integer __val, std::__true_type)
#else
			template<typename _Integer>
...
#if __GNUC__ > 3 && __GNUC_MINOR__ > 3
			template<typename _InputIterator>
			int _assign(_InputIterator __first, _InputIterator __last, std::__false_type)
#else
			template<typename _InputIterator>
			...
			{
				return const_iterator(_map[0], _map);
			}
